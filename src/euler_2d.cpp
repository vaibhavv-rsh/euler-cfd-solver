...
Euler::Euler(const std::string &params_file_name)
{
    ...
    // Initialize 2D arrays with proper dimensions
...
void Euler::init_solution()
{
    ...
    // Initialize solution with a 2D Sod shock tube
    for (int i = 0; i < this->case_parameters.number_of_points_x; i++)
    {
        for (int j = 0; j < this->case_parameters.number_of_points_y; j++)
        {
...
void Euler::MUSCL_scheme()
{
    ...
    for (int variable = 0; variable < 4; variable++)
    {
        ...
        {
            this->U_faces[0][j][FACE::WEST][variable] = U[0][j][variable];
            this->U_faces[0][j][FACE::EAST][variable] = U[0][j][variable];
        }
        // Right boundary
        for (int j = 0; j < case_parameters.number_of_points_y; j++)
        {
    ...
    for (int i = 1; i < case_parameters.number_of_points_x - 1; i++)
    {
        for (int j = 0; j < case_parameters.number_of_points_y; j++)
        {
            ...
            {
                // Calculate slopes
                double du_i_plus_half = this->U[i + 1][j][variable] - U[i][j][variable];
                double du_i_minus_half = this->U[i][j][variable] - U[i - 1][j][variable];
                ...
                // Update face values
                this->U_faces[i][j][FACE::WEST][variable] = U[i][j][variable] - 0.5 * psi_left * du_i_plus_half;
                this->U_faces[i][j][FACE::EAST][variable] = U[i][j][variable] + 0.5 * psi_right * du_i_minus_half;
            }
        }
    }

    // Interior cells - y-direction
    for (int i = 0; i < case_parameters.number_of_points_x; i++)
    {
        for (int j = 1; j < case_parameters.number_of_points_y - 1; j++)
        {
            ...
            {
                // Calculate slopes
                double dv_j_plus_half = this->U[i][j + 1][variable] - U[i][j][variable];
                double dv_j_minus_half = this->U[i][j][variable] - U[i][j - 1][variable];
                ...
                // Update face values
                this->U_faces[i][j][FACE::SOUTH][variable] = U[i][j][variable] - 0.5 * psi_left * dv_j_plus_half;
                this->U_faces[i][j][FACE::NORTH][variable] = U[i][j][variable] + 0.5 * psi_right * dv_j_minus_half;
            }
        }
    }
}
...
void Euler::update_solution()
{
    ...
    this->dt = (case_parameters.cfl * std::min(case_parameters.dx, case_parameters.dy)) / speed_max;
...
std::pair<std::array<double, 4>, std::array<double, 4>> Euler::compute_flux(int i, int j, FACE face)
{
    ...
    // Compute fluxes based on face direction
...
{
    // Update solution using both x and y fluxes
    for (int i = 0; i < this->case_parameters.number_of_points_x; i++)
    {
        for (int j = 0; j < this->case_parameters.number_of_points_y; j++)
        {
...
{
    // Check for NaN values and print warning
    std::cout << message << "\n";
    for (int i = 0; i < case_parameters.number_of_points_x; i++)
    {
        for (int j = 0; j < case_parameters.number_of_points_y; j++)
        {
            for (int k = 0; k < 4; k++)
            {
                if (std::isnan(this->U[i][j][k]) || this->U[i][j][0] < 1e-6)
                {
                    std::cout << "rho: " << this->U[i][j][0] << " at cell (" << i << "," << j << ")" << std::endl;
...
void Euler::solve()
{
    ...
    {
        // check_solution("Before update");
        this->update_solution();
        // check_solution("After update");

        // double current_time = this->case_parameters.time;
        // if (static_cast<int>(current_time*100) % 10 == 0)
        // {
        //     save_solution(filename);
        // }

        // Now do reconstruction based on scheme
        ...
        // check_solution("After compute flux");
        //  Update solution
        ...
        // if (this->case_parameters.time_step % 100 == 0)
        // {
        //     std::cout << "Time: " << this->case_parameters.time << ", dt: " << this->dt << "\n";
        // }
        // check_solution("After integrate solution in time");
    }
}
...
void Euler::print_solution()
{
    ...
    std::string output_dir = this->param_file;
    ...
    // Write solution to CSV file (2D structured grid format)
